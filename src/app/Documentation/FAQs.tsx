...
const FAQs = () => {
  ...
  return (
    <PageSection variant={PageSectionVariants.light}>
      ...
      <ExpandableSection
        toggleText={isExpanded ? 'Show less' : 'Q. Can Autotune give information about the cost of a cluster w.r.t money and resources for a customer?'}
        onToggle={onToggle}
        isExpanded={isExpanded}
        displaySize="large"
        isWidthLimited
      >
        A. We are working on the analyser recommendation manager to get complete workflow working. The current mvp focus is to optimize stack to get best performance at individual pod level. We plan to Horizontally scale in the future.
      </ExpandableSection>
      {/* Q. What are the supported SLO classes?
              </CardTitle>
      ...
      <ExpandableSection
        toggleText={isExpanded ? 'Q. In addition to goals and progress , we see that debug loops and troubleshooting are important parts of the SRE work load. Does autotune look into diagnosing and debugging paths for the SRE?Can it find the degraded service and the reason behind degradation? Basically if autotune can work beyond performance tuning and optimization' : 'Show more'}
        onToggle={onToggle}
        isExpanded={isExpanded}
        displaySize="large"
        isWidthLimited
